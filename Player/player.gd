...
func _process(delta: float) -> void:
	if Input.is_action_pressed("aim"):
		...
		smooth_camera.fov = lerp(smooth_camera.fov, smooth_camera_fov, delta * 20.0)
		weapon_camera.fov = lerp(weapon_camera.fov, weapon_camera_fov, delta * 20.0)
...
func _physics_process(delta: float) -> void:
	...
	if not is_on_floor():
		if velocity.y >= 0:
			...
			velocity.y += get_gravity().y * delta * fall_multipler
